...
shinyApp(
  ...
  server = function(input, output, session) {
    ...
    formData <- reactive({
      # Travel choices: Ensure all travel options are accounted for
      ...
      # Handle start and end dates from dateRangeInput
      ...
      data <- list(
        name = if (!is.null(input$name) && input$name != "") input$name else NA,
        dob = if (!is.null(input$dob)) input$dob else NA,
        r_num_years = if (!is.null(input$r_num_years)) input$r_num_years else NA,
        rooms = if (!is.null(input$rooms) && input$rooms > 0) input$rooms else NA,
        time_worth = if (!is.null(input$radio)) input$radio else NA,
        read = as.numeric(if (!is.null(input$read)) input$read else FALSE),  # Logical to numeric
        region = if (!is.null(input$region)) input$region else NA,
        file = if (!is.null(input$file)) input$file$name else NA,
        time_spent = if (!is.null(input$time_spent)) input$time_spent else NA,  # Use JavaScript value if available
        address_start = date_start,
        address_end = date_end
      )
      
      # Combine static data with dynamic travel options
      combined_data <- c(data, travel_data)
      
      # Convert to a data frame (ensuring one row)
      as.data.frame(combined_data, stringsAsFactors = FALSE)
    })
